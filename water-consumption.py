...
try:
    splits = int(sys.argv[1]) if len(sys.argv) > 1 else 3
except ValueError:
    print("Invalid value for 'splits', using default: 3")
    splits = 3

try:
    runtime_memory = int(sys.argv[2]) if len(sys.argv) > 2 else 2048
except ValueError:
    print("Invalid value for 'runtime_memory', using default: 2048")
    runtime_memory = 2048

try:
    ephemeral_storage = int(sys.argv[3]) if len(sys.argv) > 3 else 512
except ValueError:
    print("Invalid value for 'ephemeral_storage', using default: 512")
...
fexec = lithops.FunctionExecutor(backend=COMPUTE_BACKEND,
                                 storage=STORAGE_BACKEND,
                                 runtime_memory=runtime_memory)
print(fexec.config)
fexec.config['aws_lambda']['ephemeral_storage'] = ephemeral_storage
...
filename = "exec_ids.csv"
file_exists = os.path.isfile(filename)

# Recopilar datos adicionales, incluyendo start_time, end_time y el número de splits utilizado
runtime_memory = fexec.config['aws_lambda']['runtime_memory']
ephemeral_storage = fexec.config['aws_lambda']['ephemeral_storage']
worker_processes = fexec.config['aws_lambda']['worker_processes']
invoke_pool_threads = fexec.config['aws_lambda']['invoke_pool_threads']
vcpus = round(runtime_memory / 1769, 2)

data = {
    'execution_number': datetime.datetime.now().strftime('%Y%m%d%H%M%S'),
    'executor_id': fexec.executor_id,
    'total_price_usd': price,
    'num_files': len(storage.list_keys(bucket=DATA_BUCKET, prefix=DTM_ASC_PREFIX)),
    'input_size_gb': round(input_sz / 1_000_000_000, 2),
